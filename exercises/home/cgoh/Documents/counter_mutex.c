...
void child_code (Shared *shared)
{
    ...
    while (1) {
      ...
      sem_wait(shared->sema);
	    shared->array[shared->counter]++;
	    shared->counter++;
      sem_post(shared->sema);
...
With the use of mutex, the time increased: 5 seconds(CPU-seconds)
total times semaphre ran (wait + post): 100000000
each time ~ 50ns
*/
